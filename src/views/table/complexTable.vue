    ...
    >
      ...
      <el-table-column label="状态" class-name="status-col" width="100">
        <template slot-scope="{row}">
          ...
          <el-tag v-if="row.status==='published'" type="success">{{ row.status | statusFilter }}</el-tag>
          <el-tag v-if="row.status==='draft'" type="info">{{ row.status | statusFilter }}</el-tag>
          <!-- <el-tag v-if="row.status==='deleted'" type="danger" >{{ row.status | statusFilter }}</el-tag> -->
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm('确认删除选中记录吗？', '提示', {
        ...
        .then(() => {
          // 被选中数组传给后端接口
          batchremoveArticle(row.id).then(res => {
            // 根据后端返回类型前端提示
            this.$message({
              message: '删除成功',
              type: 'success',
